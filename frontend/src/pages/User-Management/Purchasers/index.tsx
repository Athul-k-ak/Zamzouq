import { Search, Plus, Filter, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ChevronDown } from "lucide-react";
import { StyledTable } from "@/ui/StyledTable";
import { purchasersColumns } from "@/assets/json/TableColumn";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useState } from "react";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";
import { AddPurchasersForm } from "@/components/usermanagement/purchasers/AddPurchasersForm";
import { useNavigate } from "react-router-dom";

const mockData = [
    {
        id: "P-001",
        name: "Al Barsha Mini Market",
        type: "Zamzook",
        region: "Sharjah",
        activeOrders: "145 Orders",
        earnings: "AED 3,450",
        status: "Active",
    },
    {
        id: "P-002",
        name: "Jumeirah Grocery Store",
        type: "Wholesaler",
        region: "Abu Dhabi",
        activeOrders: "145 Orders",
        earnings: "-",
...
export default function PurchasersPage() {
    ...
    const [addPurchaserTitle, setAddPurchaserTitle] = useState("Add New Purchaser");

    const handleActionClick = (row: any) => {
        navigate(`/user-management/purchasers/${row.id}`);
    };

    return (
        <div className="p-8 max-w-300 mx-auto animate-in fade-in duration-500">
            <div className="flex flex-col gap-1 mb-8">
                <h1 className="text-3xl font-bold text-slate-900">Purchasers</h1>
                <p className="text-slate-500">Manage purchaser accounts and assignments</p>
            </div>
                    ...
                    <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
                        ...
                        <DialogContent className="sm:max-w-100 bg-white rounded-3xl p-6 shadow-xl border-none overflow-hidden" showCloseButton={true}>
                            <DialogHeader className="mb-0">
                                <DialogTitle className="text-xl font-bold text-slate-900">{addPurchaserTitle}</DialogTitle>
                            </DialogHeader>
                            <AddPurchasersForm onClose={() => setIsAddModalOpen(false)} setTitle={setAddPurchaserTitle} />
                        </DialogContent>
                    </Dialog>
                </div>
            </div>
