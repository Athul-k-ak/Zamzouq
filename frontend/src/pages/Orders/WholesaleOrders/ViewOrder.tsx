...
const ViewOrder = () => {
    ...
    const orderItems = [
        { name: "Fresh Tomatoes", category: "Vegetables", unit: "KG", quantity: 50, price: "AED 5.00", total: "AED 300" },
        { name: "Fresh Tomatoes", category: "Vegetables", unit: "KG", quantity: 50, price: "AED 5.00", total: "AED 300" },
        { name: "Fresh Tomatoes", category: "Vegetables", unit: "KG", quantity: 50, price: "AED 5.00", total: "AED 300" },
        { name: "Fresh Tomatoes", category: "Vegetables", unit: "KG", quantity: 50, price: "AED 5.00", total: "AED 300" },
    ];
    ...
    return (
        <div className="p-4 lg:p-8 max-w-7xl mx-auto animate-in fade-in duration-500 pb-20">
            ...
            {/* <div className="flex items-center gap-2 text-sm text-slate-500 mb-6">
                <span className="cursor-pointer hover:text-slate-800" onClick={() => navigate("/orders/wholesale-orders")}>Orders</span>
                <ChevronRight className="h-4 w-4" />
                <span className="cursor-pointer hover:text-slate-800" onClick={() => navigate("/orders/wholesale-orders")}>Wholesale Orders</span>
                <ChevronRight className="h-4 w-4" />
                <span className="text-slate-900 font-medium">{id || "ZMZ-1234"}</span>
            </div> */}
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50/30 text-slate-400 font-medium uppercase tracking-wider text-[10px]">
                            <tr>
                                ...
                                <th className="px-6 py-4">Seller Prize</th>
                                <th className="px-6 py-4">Total(AED)</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50 border-t border-slate-100">
                            {orderItems.map((item, idx) => (
                                <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-slate-600 font-bold">{item.quantity}</td>
                                    <td className="px-6 py-4 text-slate-600 font-medium">{item.price}</td>
                                    <td className="px-6 py-4 text-slate-900 font-bold">{item.total}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="p-8 flex justify-end">
                    <div className="w-full max-w-xs space-y-4">
                        ...
                        <div className="pt-4 mt-4 border-t border-slate-100 flex justify-between items-center">
                            <span className="text-lg font-bold text-slate-900">Final Amount</span>
                            <span className="text-xl font-bold text-teal-600">AED 2,450</span>
                        </div>
                    </div>
                </div>
            </div>
